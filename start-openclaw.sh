...
WRAPPER="/root/clawd/skills/_shared/run-skill.mjs"
CRON_JOBS=""

# movie-distributor-email: 10:30 HKT = 02:30 UTC
SCRIPT="/root/clawd/skills/movie-distributor-email/scripts/check.mjs"
if [ -f "$SCRIPT" ]; then
    CRON_JOBS="${CRON_JOBS}30 2 * * 1-5 $NODE $WRAPPER \"Distributor Email\" $SCRIPT >> /tmp/distributor-email.log 2>&1; rclone copyto /tmp/distributor-email.log r2:${R2_BUCKET}/logs/distributor-email.log $RCLONE_CRON_FLAGS 2>/dev/null\n"
fi
...
if [ -f "$SCRIPT" ]; then
    CRON_JOBS="${CRON_JOBS}35 2 * * 1-5 $NODE $WRAPPER \"Movie QA Check\" $SCRIPT >> /tmp/movie-qa.log 2>&1; rclone copyto /tmp/movie-qa.log r2:${R2_BUCKET}/logs/movie-qa.log $RCLONE_CRON_FLAGS 2>/dev/null\n"
fi

# app-store-monitor: 12:00 HKT = 04:00 UTC
SCRIPT="/root/clawd/skills/app-store-monitor/scripts/check.mjs"
if [ -f "$SCRIPT" ]; then
    CRON_JOBS="${CRON_JOBS}0 4 * * 1-5 $NODE $WRAPPER \"App Store Monitor\" $SCRIPT >> /tmp/app-store.log 2>&1; rclone copyto /tmp/app-store.log r2:${R2_BUCKET}/logs/app-store.log $RCLONE_CRON_FLAGS 2>/dev/null\n"
fi

# seo-monitor: 14:00 HKT = 06:00 UTC (daily report)
SCRIPT="/root/clawd/skills/seo-monitor/scripts/seo-report.mjs"
if [ -f "$SCRIPT" ]; then
    CRON_JOBS="${CRON_JOBS}0 6 * * 1-5 $NODE $WRAPPER \"SEO Monitor (Daily)\" $SCRIPT daily >> /tmp/seo-monitor.log 2>&1; rclone copyto /tmp/seo-monitor.log r2:${R2_BUCKET}/logs/seo-monitor.log $RCLONE_CRON_FLAGS 2>/dev/null\n"
fi

# seo-monitor weekly: Monday 15:00 HKT = 07:00 UTC (weekly trend report)
if [ -f "$SCRIPT" ]; then
    CRON_JOBS="${CRON_JOBS}0 7 * * 1 $NODE $WRAPPER \"SEO Monitor (Weekly)\" $SCRIPT weekly >> /tmp/seo-monitor.log 2>&1; rclone copyto /tmp/seo-monitor.log r2:${R2_BUCKET}/logs/seo-monitor.log $RCLONE_CRON_FLAGS 2>/dev/null\n"
fi
